...
{#if data}
    ...
    {#each data.posts.data as post}
        {#if post.publish_status == "published"}
            <a href="/blog/{post.slug}" class="flex flex-col flex-reverse items-left rounded-xl mt-5 gap-4 border-2 border-solid p-5 w-full hover:border-purple-500 transition-all">
